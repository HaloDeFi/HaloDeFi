...
const HLDMaker = artifacts.require('HLDMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('HLDMaker', ([alice, bazaar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.hld = await HLDToken.new({ from: alice });
        await this.hld.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await HLDMaker.new(this.factory.address, bazaar, this.hld.address, this.weth.address);
        this.hldWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.hld.address)).logs[0].args.pair);
    ...
    it('should make HLDs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.hldWETH.address, '10000000', { from: minter });
        await this.hld.transfer(this.hldWETH.address, '10000000', { from: minter });
        await this.hldWETH.mint(minter);
        ...
        // After calling convert, bazaar should have HLD value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.hld.balanceOf(bazaar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for HLD-ETH pair
        await this.hld.transfer(this.hldWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.hldWETH.address, '100000', { from: minter });
        await this.hldWETH.sync();
        await this.hld.transfer(this.hldWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.hldWETH.address, '10000000', { from: minter });
        await this.hldWETH.mint(minter);
        assert.equal((await this.hldWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.hld.address, this.weth.address);
        assert.equal((await this.hld.balanceOf(bazaar)).valueOf(), '66249');
        assert.equal((await this.hldWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});